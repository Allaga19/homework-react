/*
 Задание к уроку 5
html нужно перенести в App.js
style.css подключайте в index.html
Элементы которые дублируются на странице сделайте отдельным компонентом
 */
...
const outputData = featuresData.map(item => 
<FeaturesSliderItem style={{ backgroundImage: `url(./images/${item.image}.svg)` }} 
                  text={item.text}
                  key={item.image} />);
